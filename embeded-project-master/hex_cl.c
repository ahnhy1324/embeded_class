...
static ssize_t hex_write (struct file *file, unsigned int __user buf,
   ...
   hex1 = 0;
   hex0 = conversion[hex_data%10];
   
   do {
      hex_data /= 10;
      if (nofill && hex_data == 0) break;
      hex0 |= conversion[hex_data % 10 & 0xF] << 8;

      hex_data /= 10;
      if (nofill && hex_data == 0) break;
      hex0 |= conversion[hex_data % 10 & 0xF] << 16;
   
      hex_data /= 10;
      if (nofill && hex_data == 0) break;
      hex0 |= conversion[hex_data % 10 & 0xF] << 24;

      hex_data /= 10;
      if (nofill && hex_data == 0) break;
      hex1 = conversion[hex_data % 10 & 0xF];

      hex_data /= 10;
      if (nofill && hex_data == 0) break;
      hex1 |= conversion[hex_data % 10 & 0xF] << 8;
   } while (0);
...
static ssize_t hex_read (struct file *file, char __user *buf,
      size_t count, loff_t *f_pos) {
